...
stage2rel=$NASC_FOLDER/../Stage2_rel_woFIH.bin.lbs
stage2dev=$NASC_FOLDER/../stage2_without_FIH.bin.lbs
...
if [ ! -f "$stage2dev" ]; then
	echo "No engineering stage2"
	exit 1
fi

find $NASC_FOLDER/SSD_In -type f -name \*.pro.lbs | xargs -i sh -c 'cat {} '$stage2rel' > {}.cat'
find $NASC_FOLDER/SSD_In -type f -name \*.dev.lbs | xargs -i sh -c 'cat {} '$stage2dev' > {}.cat'
